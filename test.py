...
if sys.platform == 'darwin':
	...
	dbPath =  homePath + '/AppData/Local/Readmoo/Local Storage/app_readmoo_0.localstorage'
else:
...
def decryptFiles(aesKey, bookId, encryptedFiles, outputRootPath):
	...
	for file in encryptedFiles:
		...
		paddingLength = 0
		print('last 16 byte: ', str(decryptBytes[decryptBytesLen-16: decryptBytesLen]))
		...
		for index in range(decryptBytesLen-1, decryptBytesLen-17, -1):
			if decryptBytes[index] == lastByte:
				...
				break;
		print('padding: ', str(lastByte), '  padding length: ', paddingLength)
...
for row in cursor:
	if row[0] == 'rsa_privateKey':
		rsaPrivateKey = row[1].decode('utf16')
	elif row[0] == '-nw-library':
		userBooksLibraryData = row[1].decode('utf16')
	elif row[0] == '-nw-access_token':
		userToken = row[1].decode('utf16')
	elif row[0] == '-nw-userid':
		userId = row[1].decode('utf16')
conn.close()
...
allBooksId = list(currentBooksInLibrary.keys())
if len(allBooksId) <= 0:
	...
	while inputNumber < 0 or inputNumber >= len(allBooksId):
		inputNumber = int(eval(input('input the number to decrypt: ')))
	#process the book
